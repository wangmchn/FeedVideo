...
/**
 寻找当前视图层级上，聚焦 container 的模块
 @discussion 寻找到的 container 可能为两种类型：
    1. UIView<FVPlayerContainer> * 直接用来添加播放器的类型
    2. UIView<FVContainerSupplier> * 为嵌套场景，即承载视图内部仍有嵌套的视图列表
 @discussion 这里因为存在嵌套的情况，找到的视图可能是两者中的一种，没法在语法上显示表明，只能用 UIView * 来承载了
 */
...
/**
 ...
 3. 横屏超过 2/3，竖屏超过 1/2 认为满足可见要求
 
 外部可通过设置该接口来提供自定义的可见性检测算法
...
/**
 ...
 @discussion block 中返回的 targetContainer 为 `UIView<FVPlayerContainer> *` or `UIView<FVContainerSupplier> *` 其中一种
